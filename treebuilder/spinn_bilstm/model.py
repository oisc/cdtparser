...
class SPINN(nn.Module):
    def __init__(self, hidden_size, proj_dropout, mlp_layers, mlp_dropout, edu_rnn_encoder_size,
        ...
        self.edu_rnn_encoder = nn.LSTM(self.wordemb_size + self.posemb_size, edu_rnn_encoder_size // 2,
                                       bidirectional=True)
        self.proj = nn.Linear(self.wordemb_size * 3 + pos_embedding_size + edu_rnn_encoder_size, self.hidden_size * 2)
    ...
    def _bilstm_edu_encode(self, word_emb, tags_emb):
        inputs = torch.cat([word_emb, tags_emb], 1).unsqueeze(1)  # (seq_len, batch, input_size)
        outputs, (h, c) = self.edu_rnn_encoder(inputs)  # h (num_directions, batch, hidden_size)
        outputs = outputs.transpose(0, 1)  # (batch, seqlen, hidden_size * num_directions)
        h = h.view(1, -1, 1)  # (batch, hidden_size * num_directions, 1)
        attn = nnfunc.softmax(torch.bmm(outputs, h), dim=1)
        output = (outputs * attn).sum(1).view(-1)
        return output, attn.view(-1)
    ...
    def node_encode(self, discourse, node_index):
        ...
        head_word = node.store['dependency'].root['word'] if node.store['dependency'].root else PAD
        ...
        w2 = word_emb[-1]
        t1 = tags_emb[0]
        head_word_id = self.word2idx[head_word] if head_word in self.word2idx else self.word2idx[UNK]
        head_word_emb = self.wordemb(torch.Tensor([head_word_id]).long()).view(-1)
        rnn_encode, _ = self._bilstm_edu_encode(word_emb, tags_emb)
        edu_emb = torch.cat([w1, w2, t1, head_word_emb, rnn_encode])
